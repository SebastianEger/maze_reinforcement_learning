...
class SimSettings:
    ...
    def __init__(self, top, functions):
        # Label
        ...
        LB2.bind('<<ListboxSelect>>', functions.LB2onSelect)
        ...
        self.E11 = Entry(frameSettings, bd =2,width = 3)
        ...
        self.E31 = Entry(frameSettings, bd =2,width = 3)
        self.E31.config(justify=CENTER)
        self.E31.insert(3,'0.2')
        ...
        B4 = Button(frameSettings, text ="Show maze", command = functions.showMaze)
        B4.grid(row=startRow, column=startColumn)

        B2 = Button(frameSettings, text ="Generate maze", command = functions.generateMaze)
        B2.grid(row=startRow, column=startColumn+1, columnspan=2)

        B1 = Button(frameSettings, text ="Start simulation", command = functions.startSim)
        B1.grid(row=startRow, column=startColumn+3, columnspan=2)
        B3 = Button(frameSettings, text ="Restart simulation", command = functions.restartSim)
        ...
        B7 = Button(frameSettings, text ="Decision maze (only NESW model)", command = functions.showDecisionMaze)
        B7.grid(row=startRow+2, column=startColumn+9, columnspan=2)

    def get_configuration(self, configuration):
        configuration['use shared Q'] = self.useQshared.get()
        configuration['learn rate'] = float(self.E31.get())
        configuration['discount'] = float(self.E32.get())
        configuration['reward goal'] = float(self.E33.get())
        configuration['reward wall'] = float(self.E34.get())
        configuration['reward robot'] = float(self.E35.get())
        configuration['reward step'] = float(self.E36.get())
        configuration['exploration rate'] = float(self.E37.get())
